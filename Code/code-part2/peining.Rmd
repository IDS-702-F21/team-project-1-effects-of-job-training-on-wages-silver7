...
```{r}
full_glm <- glm(wage ~ treat + age + educ + race + married + nodegree + re74 + re75 + 
                  treat*(re74 + re75 + married + nodegree + race + age + educ) + 
                  race*(re74 + re75 + married + nodegree + age + educ) + 
                  married*(re74 + re75 + nodegree + age + educ) +
                  nodegree*(re74 + re75 +  age + educ), data = df, family = binomial())
...
```{r}
invisible(roc(df$wage, fitted(f_aic), plot = T, print.thres = mean(as.numeric(df$wage)), legacy.axes = T, print.auc = T, col = "red3", main = "Forward AIC", cex.main = 0.9, cex.lab = 0.7, cex.axis = 0.7))

invisible(roc(df$wage, fitted(b_aic), plot = T, print.thres = mean(as.numeric(df$wage)), legacy.axes = T, print.auc = T, col = "red3", main = "Backwards AIC", cex.main = 0.9, cex.lab = 0.7, cex.axis = 0.7))

invisible(roc(df$wage, fitted(step_aic), plot = T, print.thres = mean(as.numeric(df$wage)), legacy.axes = T, print.auc = T, col = "red3", main = "Stepwise AIC", cex.main = 0.9, cex.lab = 0.7, cex.axis = 0.7))
```

```{r}
# forward AIC
cm1 <- confusionMatrix(as.factor(ifelse(fitted(f_aic) >= mean(df$wage), "1","0")),
                            as.factor(df$wage),positive = "1")
# backward AIC
cm2 <- confusionMatrix(as.factor(ifelse(fitted(b_aic) >= mean(df$wage), "1","0")),
                            as.factor(df$wage),positive = "1")
# stepwise AIC
cm3 <- confusionMatrix(as.factor(ifelse(fitted(step_aic) >= mean(df$wage), "1","0")),
                            as.factor(df$wage),positive = "1")
```
