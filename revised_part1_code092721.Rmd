...
```{r, echo=FALSE}
ggplot(policy,aes(x=age, y=treat, fill=treat)) +
  geom_boxplot() + #coord_flip() +
...
fullmodel <- lm(formula = growth ~ treat + educ_c + race + nodegree + married + re75 + educ_c*(treat + age_c + race + nodegree + married) + treat*(race + nodegree + married + age_c) + race*(nodegree + married) + age_c:married + nodegree:married, data=policy)
