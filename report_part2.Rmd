...
df <- read.csv("/Users/michellevan/Documents/mids/Stats/homework/team1/lalonde.csv")
...
knitr::kable(wage_treat, escape = FALSE, 'pipe', , caption = "Conditional table for wage given treat")
...
The final model is  $P(wage_fact = 1) = \frac{1}{1 + exp(-(\beta _{0} +  \beta_{1}treat +  \beta_{2}age + \beta_{3}educ + \beta_{4}race  +  \beta_{5}married + \beta_{6}nodegree + \beta_{7}re74  +  \beta_{8}treat*race + \beta_{9}treat*age + \beta_{10}race*re74  +  \beta_{11}race*married + \beta_{12}race*nodegree + \beta_{13}educ*race + \beta_{14}age*nodegree))}$
